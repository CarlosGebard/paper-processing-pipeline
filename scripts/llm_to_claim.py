#!/usr/bin/env python3
"""
Extract empirical claims from a single structured markdown file.
...
If a section is missing, it may contain:
_Section not found in source document._

Usage:
    export OPENAI_API_KEY="..."
    python scripts/llm_to_claim.py
    python scripts/llm_to_claim.py --input data/post_heuristics/final --output data/claims
    python scripts/llm_to_claim.py --input data/paper_001.md --output out/paper_001_claims.json
"""
...
You may extract from:
- METHODS + RESULTS
- ABSTRACT + RESULTS
- ABSTRACT + METHODS
If fewer than two sections are available, return [].
</goal>

<grounding_rules>
- Use only the provided ABSTRACT, METHODS, and RESULTS text.
- Do not use outside knowledge.
- Do not invent missing metadata.
- Do not create a claim from only one section.
...
<context_rules>
Use METHODS only to add explicit context when available:
- population
- condition
...
Use this scale:
- 0.90 to 1.00 = explicitly stated in RESULTS with clear finding and context
...
[
  {
    ...
    "support_section": "results | methods | abstract | mixed",
    ...
    "evidence_span": "exact supporting sentence(s) copied verbatim from RESULTS",
    "confidence": 0.0
  }
]
</output_format>
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Extract empirical claims from structured markdown files."
    )
    parser.add_argument(
        "--input",
        type=Path,
...
def build_prompt(
    ...
    return (
        ...
        .replace("{methods_text}", methods_text)
...
def validate_claims(data: Any) -> list[dict[str, Any]]:
    ...
    valid_support_sections = {"results", "methods", "abstract", "mixed"}
    ...
    for i, item in enumerate(data):
        ...
        if item["support_section"] not in valid_support_sections:
...
def derive_output_file(input_path: Path, output: Path) -> Path:
    ...
    if stem.endswith(".heuristics.final.md"):
        stem = stem[: -len(".heuristics.final.md")]
...
def run_claim_extraction_for_file(
    ...
    abstract_text = normalize_missing_section(sections.get("abstract"))
    methods_text = normalize_missing_section(sections.get("methods"))
    results_text = normalize_missing_section(sections.get("results"))
    available_sections_list: list[str] = []
    if abstract_text:
        available_sections_list.append("abstract")
    if methods_text:
        available_sections_list.append("methods")
    if results_text:
        available_sections_list.append("results")

    if len(available_sections_list) < 2:
        raise ValueError(
            "At least two sections are required among Abstract/Methods/Results."
        )
    ...
    prompt = build_prompt(
        ...
        max_claims=max_claims,
        available_sections=", ".join(available_sections_list),
    )
...
def run_claim_extraction_flow(
    ...
    for file_path in files:
        ...
        if output_path.exists():
            print(f"[SKIP] {file_path.name}: claims ya existen en {output_path}")
            skipped += 1
            continue
        try:
...
def main() -> int:
    args = parse_args()
    try:
        processed, skipped = run_claim_extraction_flow(
